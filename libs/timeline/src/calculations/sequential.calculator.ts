...
export class SequentialCalculator {
  ...
  static calculateForwardChain(
    ...
    let currentTime = startTime || 
      (sortedStops[0].plannedArrivalTime ? new Date(sortedStops[0].plannedArrivalTime) : new Date());
    ...
    for (let i = 0; i < sortedStops.length; i++) {
      ...
      const stopDuration = stop.plannedDuration || getDefaultStopDuration(stop.stopType);
  ...
  static updateFromStop(
    ...
    for (let i = changedStopIndex + 1; i < sortedStops.length; i++) {
      ...
      if (previousStop.calculatedDepartureTime) {
        ...
        const stopDuration = currentStop.plannedDuration || getDefaultStopDuration(currentStop.stopType);
  ...
  private static findSegmentBetweenStops(
    ...
    return segments.find(s => 
      s.originStopId === originStopId && 
      s.destinationStopId === destinationStopId
    );
  }
}